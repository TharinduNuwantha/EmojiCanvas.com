...
while($row = mysqli_fetch_assoc($result_set)){
    ...
    $fileName = str_replace($unauthorized_characters, '', $row['description']);
    $fileName = str_replace(' ', '-',$fileName);
    $file = $fileName . '-emoji.php';
    

    $siteTitle = 'Copy and Paste Emojis - emojimy.com ❤️💜 💖 💚 💛';
    $MetaTitle = $row['description'] . ' emoji ' . $row['emoji'] . ' emoji ' . $row['emoji'] .$row['emoji']. ' copy and paste '.$row['emoji']. ' emoji ' . $row['emoji'] . ' emoji ' . $row['emoji'];
    $MetaDis = $row['description'] . ' emoji ' . $row['emoji'].$row['emoji'].$row['emoji'].$row['emoji']. ' Copy and paste & Search for any emojis '.$row['emoji'].$row['emoji']. $row['emoji'].'('.$row['category'].')';
    $mtLst = '';
...
while(file_exists($file)){
    $unauthorized_characters = array('-emoji.php');
    $fileName = str_replace($unauthorized_characters, '', $file);
    $fileName = str_replace(' ', '-',$fileName);
    $file = $fileName .'-'.$file_No.'-emoji.php';
    $file_No++;
    echo "Same File Detected. ID : ".$row['id'].'Emoji : '.$row['emoji'].'Old File Name : '.$oldFile.' // New File Name : '.$file.'<br>';
}
$file_No = 1;
file_put_contents($file, $data);
    ...
    if(file_exists($file)) {
        ...
        echo "<p style='color: rgb($randomNumber1, $randomNumber2, $randomNumber3); float: left;background-color: rgb($, $randomNumber2, $randomNumber3,$randomNumber1);padding: 5px;'>Data has been written to the file successfully.</p>";
    } else {
        echo "Error writing to the file. ID : ".$row['id'].'Name : '.$row['description'].'Emoji : '.$row['emoji'].'<br>';
    }
...
// } else {
//     echo "Error writing to the file. ".$row['id'].'<br>';
// }
